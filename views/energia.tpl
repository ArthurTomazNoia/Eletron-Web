...
<head>
    ...
    <style>
        ...
        .title{font-size:3em;font-weight:900;text-transform:uppercase;letter-spacing:2px;border:2px solid #9400D3;text-shadow:0 0 8px rgba(148,0,211,0.7);padding:10px 25px;margin-bottom:40px;}
...
<body>
    ...
    <script>
        ...
        // Adiciona um "ouvinte" de clique para o botão LIGAR
        botaoLigar.addEventListener('click', async function() {
            try {
                // 1. Envia uma requisição POST para o endpoint /energia/on
                const response = await fetch('/energia/on', { method: 'POST' });
                
                if (!response.ok) {
                    throw new Error('Falha ao ligar o alarme.');
                }

                // 2. Se a requisição for bem-sucedida, avisa o usuário e redireciona
        ...
        // Adiciona um "ouvinte" de clique para o botão DESLIGAR
        botaoDesligar.addEventListener('click', async function() {
            try {
                // 1. Envia uma requisição POST para o endpoint /energia/off
                const response = await fetch('/energia/off', { method: 'POST' });

                if (!response.ok) {
                    throw new Error('Falha ao desligar o alarme.');
                }
